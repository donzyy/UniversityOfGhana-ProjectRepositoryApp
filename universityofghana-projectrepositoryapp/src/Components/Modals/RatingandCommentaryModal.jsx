...
import { VscCheck, VscCommentDiscussion, VscStarEmpty } from "react-icons/vsc";
...
const RatingandCommentaryModal = ({submissionID}) => {
  ...
  const handleSubmit = () => {
    handleRatingandCommentary({
      submissionID: student_Submission_ID,
  ...
  const handleRatingandCommentary = async ({submissionID}) => {
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger>
        ...
        <button onClick={()=> handleRatingandCommentary(submission.student_Submission_ID)} className="text-LegonBlue hover:text-purple-600">
          ...
          <div className='bg-white rounded-md shadow-lg px-4 py-6'>
            ...
            <div className='items-center gap-2 mt-3 text-sm sm:flex'>
              <Dialog.Close asChild>
                <button className='w-full mt-2 p-2.5 flex-1 text-white bg-LegonGold rounded-md outline-none ring-offset-2 ring-LegonBlue focus:ring-2'>Submit</button>
              </Dialog.Close>
...
export default RatingandCommentaryModal;